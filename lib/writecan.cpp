...
bool WriteUserInputToCan(SocketCan &socket, database_type::Database &db, const int &msdelay){
    ...
    const CanFrame ignition = ConvertToCanFrame<database_type::Ignition, can_data_base::StartButton>(db_ignition, cb_ignition);

    can_data_base::GearPosition cb_gear; 
    database_type::Gear db_gear = db.gear; 
    const CanFrame gear = ConvertToCanFrame<database_type::Gear, can_data_base::GearPosition>(db_gear, cb_gear);

    can_data_base::PedalPosition cb_gas; 
    unsigned int db_gas = db.gas; 
    const CanFrame gas = ConvertToCanFrame<unsigned int, can_data_base::PedalPosition>(db_gas, cb_gas);
